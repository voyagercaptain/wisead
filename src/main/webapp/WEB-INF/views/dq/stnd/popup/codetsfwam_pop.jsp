...
function initGrid()
{
    ...
    with(grid_pop){
    	...
    	var headtext  = "No.|<s:message code='META.HEADER.CODETSFWAM.POP.1'/>";
    	headtext += "|<s:message code='META.HEADER.CODETSFWAM.POP.2'/>"
		headtext += "|<s:message code='META.HEADER.CODETSFWAM.POP.3'/>";
		headtext += "|<s:message code='META.HEADER.CODETSFWAM.POP.4'/>";
		headtext += "|<s:message code='META.HEADER.CODETSFWAM.POP.5'/>";
		//headtext  = "No.|상태|선택";
    	//headtext += "|타겟DB_ID|타겟DB명|타겟스키마ID|타겟스키마명"
		//headtext += "|도메인논리명|도메인물리명|코드유형|대분류코드|코드ID|코드값|코드값명|상위코드id|상위코드값|표시순서|사용여부";
		...
		//headtext += "|적요1|적요2";
	     ...
	     var cols = [						
	     			...
	     			{Type:"Text",   Width:60,  SaveName:"cdVal",   	Align:"Left", Edit:0, },
	     			{Type:"Text",   Width:80,  SaveName:"cdValNm",   	Align:"Left", Edit:0},
...
function doAction(sAction)
{
    ...
    {		    
        ...
        case "Search":
        	...
        	grid_pop.DoSearch('<c:url value="/dq/stnd/getcdtsfwam_pop.do" />', param);
...
// 			alert(saveJson.Code); 처리대상 행이 없는 경우 리턴한다.
        	...
        	param += "&rqstDcd=CU";
        	//param += "&tgtDbConnTrgPnm="+$("#dbConnTrgId option:selected").text();
        	//param += "&tgtDbConnTrgId="+$("#dbConnTrgId option:selected").val();
        	//param += "&tgtDbSchPnm="+$("#dbSchId option:selected").text();
        	//param += "&tgtDbSchId="+$("#dbSchId option:selected").val();
