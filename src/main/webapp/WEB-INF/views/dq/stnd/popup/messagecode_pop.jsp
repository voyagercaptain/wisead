...
function initGrid()
{
    ...
    with(grid_pop){
        ...
        var headtext = "No.|상태|선택|조치코드ID|조치코드|언어구분코드|조치코드사용여부|조치코드삭제여부|조치코드적용일자|조치내용";			
...
function doAction(sAction)
{
    ...
    {		    
        case "Search":
        	...
        	grid_pop.DoSearch('<c:url value="/meta/stnd/getMessageCodeList.do" />', param);
...
// 			alert(saveJson.Code); 처리대상 행이 없는 경우 리턴한다.
        	...
        	param += "&rqstDcd=CU";
        	var url = "<c:url value="/meta/stnd/regWam2WaqMessageCode.do" />";
...
function grid_pop_OnDblClick(row, col, value, cellx, celly) {
	...
	//iframe 형태의 팝업일 경우
	if ("${search.popType}" == "I") {
		parent.returnCodePop(JSON.stringify(retjson));
	} else {
		opener.returnCodePop(JSON.stringify(retjson));
	}
                   ...
                   <tbody>
                   		<tr>                          
                            ...
                            <td>
                                ...
                                <input type="text" name="actnCd" id="actnCd" />
                                </span>
                            </td>  
                       </tr>
                   </tbody>
