...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext  = "No.|상태|선택|요청구분|등록유형";
        	headtext += "|AS-IS오류코드|오류전문언어구분코드|오류유형코드|메시지유형|메시지구분|업무구분|표준오류코드여부|오류코드|전산메세지오류원인내용|고객메세지오류원인내용|오류코드적용일자|조치코드|담당파트|담당자명";
        ...
        var cols = [                        
					...
					{Type:"Text",   Width:100,  SaveName:"stsyErrCd",  Align:"Left", Edit:0, KeyField:0},
					{Type:"Combo",   Width:100,  SaveName:"errTlgLanDscd",  Align:"Left", Edit:0, KeyField:0,Hidden:1},
					{Type:"Combo",   Width:100,  SaveName:"errTpCd",  Align:"Left", Edit:0, KeyField:0,Hidden:1}, 
					{Type:"Combo",   Width:100,  SaveName:"msgTyp",  Align:"Left", Edit:0, KeyField:1}, 
					{Type:"Combo",   Width:100,  SaveName:"msgDit",  Align:"Left", Edit:0, KeyField:1}, 
					{Type:"Combo",   Width:100,  SaveName:"bizDit",  Align:"Left", Edit:0, KeyField:1}, 
					{Type:"Combo",   Width:100,  SaveName:"stdErrCdYn",  Align:"Left", Edit:0, KeyField:0, Hidden:1},  
					{Type:"Text",   Width:100,  SaveName:"errCd",  Align:"Left", Edit:0, KeyField:0},
					{Type:"Text",   Width:200,  SaveName:"isdErrCasCts",  Align:"Left", Edit:0, KeyField:1}, 
					{Type:"Text",   Width:200,  SaveName:"osdErrCasCts",  Align:"Left", Edit:0, KeyField:0, Hidden:1},
					...
					{Type:"Text",   Width:100,  SaveName:"tskDmn",	Align:"Left", 	 Edit:0, KeyField:1},
					{Type:"Text",   Width:100,  SaveName:"mngUser",	Align:"Left", 	 Edit:0, KeyField:1},
                ];
...
//  	 	   	if(SaveJson.data.length == 0) return;
 	 	    ...
 	 	    var url = "<c:url value="/meta/stnd/regmsgstdrqstlist.do"/>";
 	 		//iframe 형태의 팝업일 경우
        	var param = "";
        	if ("${search.popType}" == "I") {
...
function doSave()
{
	...
	for (var i=1;i<=grid_sheet.GetTotalRows();i++){
		...
		if(grid_sheet.GetCellValue(i,"errCd") == "" && grid_sheet.GetCellValue(i,"regTypCd") != 'D'){
			grid_sheet.SetCellValue(i,"errCd",grid_sheet.GetCellValue(i,"msgTyp")+grid_sheet.GetCellValue(i,"msgDit")+grid_sheet.GetCellValue(i,"bizDit")+"0001");   
			grid_sheet.SetCellValue(i,"regTypCd","C");   
		}else if (grid_sheet.GetCellValue(i,"regTypCd") == 'D'){
			grid_sheet.SetCellValue(i,"regTypCd","D");   
		}else{
			grid_sheet.SetCellValue(i,"regTypCd","U");   
		}
		if(grid_sheet.GetCellValue(i,"regTypCd") != 'D' && grid_sheet.GetCellValue(i,"msgTyp").toUpperCase() == "D"  && grid_sheet.GetCellValue(i,"bizDit").toUpperCase() != "COR")
		{ 	
			alert("화면메시지 인경우 업무구분을 'COR'만 등록가능합니다."); 
			grid_sheet.SelectCell(i, 0);
			return;
		} 

		if(grid_sheet.GetCellValue(i,"errCd"      	) =="" && grid_sheet.GetCellValue(i,"regTypCd") !="C"){
			alert("변경,삭제시에는 "+i+"열의  오류코드를 입력해야 합니다."); 
			grid_sheet.SelectCell(i, 0);
			return;
		}
	}
	for (var i=1;i<=grid_sheet.GetTotalRows();i++){
		for(var j=1;j<=grid_sheet.GetTotalRows();j++){
			dupRow = grid_sheet.ColValueDup("errCd");
			if(dupRow != -1){
				tmp = grid_sheet.GetCellValue(dupRow,"errCd");
				grid_sheet.SetCellValue(dupRow,"errCd",tmp.substring(0, 5)+String(pad(Number(tmp.substring(6,9))+1,4)));
			}
		}
...
<div class="pop_tit" >
    ...
    <div class="pop_tit_txt">엑셀업로드-표준오류메시지</div>
