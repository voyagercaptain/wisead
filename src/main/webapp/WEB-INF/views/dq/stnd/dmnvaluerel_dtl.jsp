...
// 	grid_sub_dmnvaluerel.FitColWidth();
});


function initsubgrid_dmnvaluerel() {

    with(grid_sub_dmnvaluerel){
...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        var headers = [
                    {Text:"<s:message code='META.HEADER.DMNVALUEREL.DTL'/>", Align:"Center"}
                ];
                //No.|유효값관계예정1|변경예정2|변경예정3
...
//         SetColHidden("objVers",1);
   	...
   	//==시트설정 후 아래에 와야함=== 
    init_sheet(grid_sub_dmnvaluerel);    
    //===========================
  	
    //저장 처리 과정을 디버깅 메시지를 팝업으로 표시 (-1)
//     grid_sub_dmnvaluerel.ShowDebugMsg(-1);	
...
y : y좌표
*/
function grid_sub_dmnvaluerel_OnDblClick(row, col, value, cellx, celly) {

	if(row < 1) return;
	
}

function grid_sub_dmnvaluerel_OnClick(row, col, value, cellx, celly) {
...
function grid_sub_dmnvaluerel_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<!-- 그리드 입력 입력 -->
	<div class="grid_01">
	     <script type="text/javascript">createIBSheet("grid_sub_dmnvaluerel", "100%", "250px");</script>
	</div>
	<!-- 그리드 입력 입력 End -->
