...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        var headers = [
                    {Text:"<s:message code='META.HEADER.SDITMWHEREUSED.DTL'/>", Align:"Center"}
                ];
...
//                  {Type:"CheckBox", Width:60, SaveName:"ibsCheck",    Align:"Center", Edit:1, Hidden:0},
        ...
        InitColumns(cols);
      	//콤보 목록 설정...
      	SetColProperty("objType", 	{ComboCode:"STWD|INFO|SDITM|TABLE|DMN|SYMN|INDEX|TBLSPAC", 	ComboText:"<s:message code="STRD.WORD" />|<s:message code="INFO.TYPE" />|<s:message code="STRD.TERMS" />|<s:message code="TBL" />|<s:message code="DMN" />|<s:message code="SIMIWORD"/>|<s:message code="IDEX"/>|<s:message code="TBL.SPACE"/>"}); 
...
function grid_sub_sditmwhereused_OnDblClick(row, col, value, cellx, celly) {
	...
	var objId = grid_sub_sditmwhereused.GetCellValue(row, "objId");	
	if (grid_sub_sditmwhereused.GetCellValue(row, "objType") == 'STWD') {
		window.open().location.href = "<c:url value="/dq/stnd/stwd_lst.do"/>"+"?objId="+objId;	
	} else if (grid_sub_sditmwhereused.GetCellValue(row, "objType") == 'DMN') {
		window.open().location.href = "<c:url value="/dq/stnd/dmn_lst.do"/>"+"?objId="+objId;
	} else if (grid_sub_sditmwhereused.GetCellValue(row, "objType") == 'SDITM') {
		window.open().location.href = "<c:url value="/dq/stnd/sditm_lst.do"/>"+"?objId="+objId;
	} else if (grid_sub_sditmwhereused.GetCellValue(row, "objType") == 'TABLE') {
		window.open().location.href = "<c:url value="/dq/model/pdmtbl_lst.do"/>"+"?pdmTblId="+objId;
	} else if (grid_sub_sditmwhereused.GetCellValue(row, "objType") == 'INFO') {
// 		location.href = "stwd_lst.do?objId="+objId;
	} else if (grid_sub_sditmwhereused.GetCellValue(row, "objType") == 'SYMN') {
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<!-- 그리드 입력 입력 -->
	<div class="grid_01">
	     <script type="text/javascript">createIBSheet("grid_sub_sditmwhereused", "100%", "150px");</script>
	</div>
	<!-- 그리드 입력 입력 End -->
