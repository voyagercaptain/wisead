...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
		...
		case "DownAllExcel":
			$("#frmSearch").attr('action','<c:url value="/dq/stnd/sdDmnExcel.do"/>').submit();
			break;
...
// 			getRqstVrfLst(param);
		...
		case "Decide":  //확정
			...
			showMsgBox("CNF", "<s:message code="MSG.CONFIRM" />", function (code) {
				...
				var url = '<c:url value="/dq/stnd/decideStndDmn.do"/>';
				var param = $('form[name=frmSearch]').serialize();
				//var chkYn = $('input:checkbox[id="chkYn"]:checked').val();
				//param = param + "&chkYn="+chkYn;
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  ...
	  <div id="tabs-DMN">
			<div id="detailInfoDMN"><%@include file="exl/orgstnddmn_exl.jsp" %></div>
	  </div>
