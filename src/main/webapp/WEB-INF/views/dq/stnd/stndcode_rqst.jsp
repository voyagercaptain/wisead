...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext = "<s:message code='META.HEADER.CD.DFNT.RQST1'/>";
        headtext += "<s:message code='META.HEADER.CD.DFNT.RQST2'/>";
        headtext += "<s:message code='META.HEADER.CD.DFNT.RQST3'/>";
...
//     			grid_sheet.DataInsert(0);
//     		}
    	    var fileName="코드정의서(기관표준).xlsx";
    		grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, Mode:2, FileName : fileName});
        	break;

        case "Search":

        	var url = '<c:url value="/dq/stnd/getCodelist.do"/>';
 	 	...
 	 	case 'Save' : //검증
			...
			var row = grid_sheet.ColValueDup("orgNm|commCdNm|cdVal|cdValNm");
			var rows = grid_sheet.ColValueDupRows("orgNm|commCdNm|cdVal|cdValNm");
			...
			var url = '<c:url value="/dq/stnd/regCodeWamlist.do"/>';
    	...
    	case "Delete" :
			...
			var url = '<c:url value="/dq/stnd/delCodeWamlist.do"/>';
