...
function grid_STWD_init() {
    ...
    with(grid_STWD){
		...
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|STWDID|기관명|DB명|표준단어명|영문약어명|단어영문명|단어설명|형식단어여부|도메인분류명|이음동의어목록|금칙어목록|등록일자|요청자ID|요청자명|요청번호|요청일련번호
 		...
 		var headtext  = "No.|상태|선택|번호|제정차수|검토상태|검토내용|요청구분|등록유형|검증결과|STWDID|공통표준단어명|공통표준단어영문약어명|공통표준단어영문명|공통표준단어설명|형식단어여부|공통표준도메인분류명|이음동의어 목록|금칙어 목록|관리부서명|제정일자|요청자ID|요청자명|요청번호|요청일련번호|특이사항|제개정내용|사용여부";
		...
		var cols = [						
					...
					{Type:"Text",   Width:100,  SaveName:"ownrOrg",      Align:"Left", Edit:1, Hidden:1},  // 소관기관명
