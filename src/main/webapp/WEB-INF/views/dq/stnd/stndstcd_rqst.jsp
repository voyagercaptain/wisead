...
// 			getRqstVrfLst(param);
		...
		case "Inspect":  //검증
			...
			var url = '<c:url value="/dq/stnd/inspectStndCode.do"/>';
		...
		case "Decide":  //확정
			...
			showMsgBox("CNF", "<s:message code="MSG.CONFIRM" />", function (code) {
				...
				var url = '<c:url value="/dq/stnd/decideStndCode.do"/>';
				var param = $('form[name=frmSearch]').serialize();
		...
		case "Init":  //초기화
			...
			//프로파일별 url 셋팅
			var url = "";
			url = '<c:url value="/dq/stnd/initStndCode.do"/>';
    	...
    	case "Save":  //임시저장
			...
			var url = '<c:url value="/dq/stnd/regStndCodeWamlist.do"/>';
		...
		<div class="divLstBtn" style="display: none;">  
            <div class="bt03">
			    ...
			    <c:if test="${sessionScope.loginVO.usergId eq 'OBJ_00000034586'}">
					...
					<button class="btn_save" id="btnSave" 	name="btnSave"><s:message code="BTN.TEMP.SAVE" /></button> <!-- 저장 -->
