    ...
    with(grid_sheet){
        ...
        if(HeaderCnt != ""){
    		...
    		for(var i=1; i<=HeaderCnt; i++){
    			colNm = "colNm" + i;
    			cols.push(  {Type:"Text", Width:"120",  SaveName:colNm, Align:"Center", Edit:0}  ) ;
    		} 
...
function grid_sheet_OnDblClick(row, col, value, cellx, celly) {
	...
	var param = "?objId="+"${search.prfId}";
    param += "&objDate="+"${search.anaStrDtm}".replace(/ /g, ''); 
...
<form id="frmSearch" name="frmSearch" method="post">
    <input  type="hidden"  name="objId" id="objId" value="${headerVO.objId}" />
    <input  type="hidden"  name="objIdCol" id="objIdCol" value="${headerVO.objIdCol}" />
    <input  type="hidden"  name="objResTbl" id="objResTbl" value="${headerVO.objResTbl}" />
    <input  type="hidden"  name="objErrTbl" id="objErrTbl" value="${headerVO.objErrTbl}" />
    <input  type="hidden"  name="erDataSnoCol" id="erDataSnoCol" value="${headerVO.erDataSnoCol}" />
    <input  type="hidden"  name="objDate" id="objDate" value="${headerVO.objDate}" />
    <input  type="hidden"  name="objGb" id="objGb" value="${headerVO.objGb}" />
    <input  type="hidden"  name="colNm" id="colNm" value="${headerVO.colNm}" />
</form>
