...
<script type="text/javascript">
	$(document).ready(	function() {
		...
		//부모테이블 진단대상 정보 셋팅
		$("form[name=frmSearch] #schPaDbConnTrgLdm").val(  "${wamReacTblVO.paTblDbConnTrgNm }" == "" ? "${anaTrgTblVO.dbConnTrgPnm }" : "${wamReacTblVO.paTblDbConnTrgNm }");
		$("form[name=frmSearch] #schPaDbConnTrgId").val(  "${wamReacTblVO.paTblDbConnTrgId }" == "" ? "${anaTrgTblVO.dbConnTrgId }" : "${wamReacTblVO.paTblDbConnTrgId }");
		$("form[name=frmSearch] #schPaDbSchNm").val(  "${wamReacTblVO.paTblDbcSchNm }" == "" ? "${anaTrgTblVO.dbSchPnm }" : "${wamReacTblVO.paTblDbcSchNm }");
		$("form[name=frmSearch] #schPaDbSchId").val(  "${wamReacTblVO.paTblDbcSchId }" == "" ? "${anaTrgTblVO.dbSchId }" : "${wamReacTblVO.paTblDbcSchId }");
...
// 		$("form[name=frmSearch] input[type=text]").css("border-color","transparent");
	...
	$(window).resize(function() {
		...
		//관계컬럼
		grid_reac_col_sheet.FitColWidth();
	});
		...
		with (grid_catbl) {
			...
			//선택|No|상태|Position|진단대상ID|진단대상명|스키마ID|스키마명|자식테이블명|자식테이블한글명|자식컬럼명|자식컬럼한글명|Pk여부|Data Type|Null여부|Default
			 ...
			 var headerInfo = {Sort:1, ColMove:0, ColResize:1, HeaderCheck:1};
	           ...
	           var cols = [
	                   {Type:"Combo", Width:70,   SaveName:"ibsCheck",    Align:"Center", Edit:1, Hidden:0},
			...
			SetColProperty("ibsCheck", 	{ComboCode:"|1|2|3|4|5", ComboText:"|1|2|3|4|5"});
...
//	 		SetColProperty("rvwStsCd", ${codeMap.rvwStsCd});
	...
	function doAction(sAction, param) {
		...
		switch (sAction) {
			...
			//관계컬럼 조회
			case "SearchReacColLst": 
				var param = "&prfId="+$('form[name=frmSearch] #prfId').val() ; 
				grid_reac_col_sheet.DoSearch("<c:url value="/dq/profile/getPrfReacReacColLst.do" />", param);
				break;
				
			case "InsertReacCol":
				if(grid_catbl.ColValueDup("ibsCheck", {"IncludeEmpty" : 0}) == -1 && grid_patbl.ColValueDup("ibsCheck", {"IncludeEmpty" : 0}) == -1) {
					var cTotRow = grid_catbl.GetDataLastRow();
					var pTotRow = grid_patbl.GetDataLastRow();
					
					for(i=1; i<6; i++) {
						if(cTotRow < i || pTotRow < i) break;
						
						var cRow = grid_catbl.FindText("ibsCheck", "" + i);
						var pRow = grid_patbl.FindText("ibsCheck", "" + i);
						
						if(cRow == -1 || pRow == -1) break;
						...
						if(parent.grid_reac_col_sheet.FindText("chTblDbcColNm", grid_catbl.GetCellValue(cRow, "dbcColNm"),0,-1,0) == -1
								|| parent.grid_reac_col_sheet.FindText("paTblDbcColNm", grid_patbl.GetCellValue(pRow, "dbcColNm"),0,-1,0) == -1) {
							var iRow = grid_reac_col_sheet.DataInsert(0);
							form2ibsmapping(iRow, $("form[name=frmInput]"), grid_reac_col_sheet);
						}
			...
			//parent 화면 셋팅
			case "Return":
				if(grid_reac_col_sheet.GetTotalRows() == 0){
				...
				//iframe 형태의 팝업일 경우
				if ("${anaTrgTblVO.popType}" == "I") {
					parent.returnReacColPop(JSON.stringify(retjson));
				} else {
					opener.returnReacColPop(JSON.stringify(retjson));
				}
				
				//관계컬럼 반영
				ibsSheet2SheetStatus(grid_reac_col_sheet, parent.grid_reac_col_sheet, "ibsStatus"); 
	...
	/*체크박스 변경시 발생 이벤트*/
	function grid_reac_col_sheet_OnBeforeCheck(Row, Col) {
		if(grid_reac_col_sheet.GetCellValue(Row,"ibsStatus") == "I") {
			//선택된 row  radio 버튼 edit true
			var cfRow = grid_catbl.FindText("dbcColNm", grid_reac_col_sheet.GetCellValue(Row,"chTblDbcColNm"));
			var pfRow = grid_patbl.FindText("dbcColNm", grid_reac_col_sheet.GetCellValue(Row,"paTblDbcColNm"));
			grid_catbl.SetCellEditable(cfRow, "ibsCheck", 1);
			grid_patbl.SetCellEditable(pfRow, "ibsCheck", 1);
			//선택된 row 삭제
			grid_reac_col_sheet.RowDelete(Row, 0);
		}
	 }
	...
	function grid_catbl_OnSearchEnd(code, message, stCode, stMsg) {
		...
		if (code < 0) {
			...
			var param = "&schDbConnTrgLdm="+$('form[name=frmSearch] #schPaDbConnTrgLdm').val();
			     ...
			     param += "&prfId="+$('form[name=frmSearch] #prfId').val();
	...
	function grid_reac_col_sheet_OnSearchEnd(code, message, stCode, stMsg) {
		...
		if (code < 0) {
			...
			for(var i=1; i<=grid_reac_col_sheet.GetTotalRows(); i++){
				//자식컬럼
				cRow = grid_catbl.FindText("dbcColNm", grid_reac_col_sheet.GetCellValue(i, "chTblDbcColNm"));
				if(cRow > 0){
					grid_catbl.SetCellEditable(cRow, "ibsCheck", 0);
					grid_catbl.SetCellValue(cRow, "ibsCheck", "0");
				}
				//부모컬럼
				pRow = grid_patbl.FindText("dbcColNm", grid_reac_col_sheet.GetCellValue(i, "paTblDbcColNm"));
				alert(pRow);
				if(pRow > 0){
					grid_patbl.SetCellEditable(pRow, "ibsCheck", 0); 
					grid_patbl.SetCellValue(pRow, "ibsCheck", "0");
	...
	<div class="pop_tit">
		...
		<div class="pop_content">
			...
			<div id="search_div">
				...
				<input type="hidden" name="prfId" id="prfId" value="${wamReacTblVO.prfId}" />  
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0"	summary="부모테이블 조회">
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<input type="text" class="wd60p" name="schPaDbcTblNm" 	id="schPaDbcTblNm" value="${wamReacTblVO.paTblDbcTblNm }" />
...
<!-- 												<button class="btn_search" id="btnTblSearch" name="btnTblSearch"  style="float:right" >테이블<s:message code="INQ"/>
 -->
										...
										<tr>
											...
											<td><input type="text" class="wd60p" name="schPaDbcTblKorNm" 	id="schPaDbcTblKorNm" value="${wamReacTblVO.paTblDbcTblKorNm }" /></td>
										</tr>
									</tbody>
								</table>
								</div>
							...
							<tr>
								<td>
									<div id="grid_01" class="grid_01">
										<script type="text/javascript">createIBSheet("grid_catbl", "98%", "500px");</script>
									</div>
								</td>
								<td>
									<div style="clear: both; width: 4px;"><span></span></div>
								...
								<td>
									<div id="grid_02" class="grid_01">
										<script type="text/javascript"> createIBSheet("grid_patbl", "98%", "500px");	</script>
									</div>
								</td>
							</tr>
						</tbody>
			...
			<div style="clear: both; height: 5px;"><span></span></div>
			<div style="display:none;" id="grid_03" class="grid_01">
				<script type="text/javascript"> createIBSheet("grid_reac_col_sheet", "100%", "210px");	</script>
			</div>
