...
$(window).load(function(){
	...
	setibsTabHeight($("#grid_05"), 100);
});


$(window).resize( function(){
    	//그리드 가로 스크롤 방지
//     	grid_pc03.FitColWidth();
    }
);

function grid_pc03_excel()
{
	grid_pc03.Down2Excel({HiddenColumn:1, Merge:1});
}


function initprfpc03grid() {
    ...
    with(grid_pc03){
        ...
        var headers = [
                       {Text:"<s:message code='DQ.HEADER.DATEANA_LST'/>", Align:"Center"}
                   ];
                   //No.|상태|선택|최근분석차수|최근분석일시|진단대상ID|스키마ID|진단대상명|스키마명|테이블명|테이블한글명|순서|컬럼명|컬럼한글명|날짜형식유형|사용자날짜형식|분석총건수|추정오류건수|추정오류율(%)|DPMO|SIGMA|프로파일ID|작업ID|작업명
           ...
           var cols = [                        
							...
							{Type:"Text",   Width:150,  SaveName:"dateFrmCd",    	Align:"Left", Edit:0, Hidden:0},
							{Type:"Text",   Width:150,  SaveName:"userDateFrm",    	Align:"Left", Edit:0, Hidden:0},
   	...
   	//==시트설정 후 아래에 와야함=== 
    init_sheet(grid_pc03);    
    //===========================
  	
    //저장 처리 과정을 디버깅 메시지를 팝업으로 표시 (-1)
//     grid_pc03.ShowDebugMsg(-1);	
...
y : y좌표
*/
function grid_pc03_OnDblClick(row, col, value, cellx, celly) {
	if(row < 1) return;
	var param = "dbConnTrgId="+grid_pc03.GetCellValue(row, "dbConnTrgId");
	 param += "&dbSchId="+grid_pc03.GetCellValue(row, "dbSchId");
	 param +="&dbcTblNm="+grid_pc03.GetCellValue(row, "dbcTblNm");
	 param +="&dbcColNm="+grid_pc03.GetCellValue(row, "dbcColNm");
	 param +="&prfId="+grid_pc03.GetCellValue(row, "prfId");
	 param +="&anaDgr="+grid_pc03.GetCellValue(row, "anaDgr");
	 param +="&anaStrDtm="+grid_pc03.GetCellValue(row, "anaStrDtm");
	 param +="&prfKndCd="+"PC03";
...
function grid_pc03_OnClick(row, col, value, cellx, celly) {
	if(row < 1) return;
	
	//분석일시 정보를 위한 선택된 row 번호 저장
// 	$("#frmSearch #sheetRow").val(row);
// 	$("#frmSearch #anaStrDtm").val(grid_pc03.GetCellValue(row, "anaStrDtm"));
// 	$("#frmSearch #prfId").val(grid_pc03.GetCellValue(row, "prfId"));
// 	$("#frmSearch #prfKndCd").val("PC03");

	anaStrDtms = grid_pc03.GetCellValue(row, "anaStrDtm");
	objId = grid_pc03.GetCellValue(row, "prfId");
	prfKndCds = "PC03";
}

function grid_pc03_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<div class="grid_01" id="grid_05">
     <script type="text/javascript">createIBSheet("grid_pc03", "100%", "420px");</script>
</div>
