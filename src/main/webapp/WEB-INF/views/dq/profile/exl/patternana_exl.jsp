...
function grid_pc05_init() {
    ...
    with(grid_pc05){
        ...
        var headers = [
                       {Text:"<s:message code='DQ.HEADER.PATTERNANA_EXL'/>", Align:"Center"}
                   ];
           //No.|상태|선택|요청번호|요청일련번호|요청상세일련번호|검토상태|검토내용|요청구분|등록유형|검증결과|진단대상명|스키마명|테이블명|컬럼명|사용자정의패턴|추가조건
           ...
           var cols = [                        
                       ...
                       {Type:"Text",   Width:50,  SaveName:"rqstNo",    	Align:"Right", Edit:0, Hidden:1}, 			//요청번호
                       {Type:"Text",   Width:50,  SaveName:"rqstSno",    	Align:"Right", Edit:0, Hidden:1}, 			//요청일련번호
                       {Type:"Text",   Width:50,  SaveName:"rqstDtlSno",    	Align:"Right", Edit:0, Hidden:1}, 		//요청상세일련번호
                       ...
                       {Type:"Text",   Width:100,  SaveName:"userDfPtr",    	Align:"Center", Edit:1}, 	//사용자정의패턴
...
function grid_pc05_OnLoadExcel() {
}
...
function grid_pc05_OnDblClick(row, col, value, cellx, celly) {
	...
	if(grid_pc05.GetCellValue(row, "vrfCd") == 2) {
...
function grid_pc05_OnClick(row, col, value, cellx, celly) {
	...
	//선택한 상세정보를 가져온다...
	var param =  grid_pc05.GetRowJson(row);

	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '<s:message code="STRING.PTRN.ANLY"/> : ' + param.dbConnTrgPnm + '.' + param.dbSchPnm +'.'+ param.objNm; /*문자열패턴분석*/
	$('#pc05_sel_title').html(tmphtml);
	
	//검증결과 검증오류일경우가 아닐경우 RETURN
	if( grid_pc05.GetCellValue(row, "vrfCd") != "2")  return;
...
function grid_pc05_OnSaveEnd(code, message) {
	doAction("Search"); 
}
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<!-- 그리드 입력 입력 -->
<div id="grid_pc05" class="grid_01">
     <script type="text/javascript">createIBSheet("grid_pc05", "100%", "400px");</script>
</div>

<!-- 그리드 하단 영역 : 레코드 선택시 내용 표시 및 수정 가능하도록 -->
<div class="selected_title_area" id="selected_title_area">
	<div class="selected_title" id="pc05_sel_title"></div>
</div>
