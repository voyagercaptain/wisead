    ...
    with(grid_sheet){
      ...
      /*   No.|선택|상태|스케줄명(물리명)|스케줄명|스케줄유형코드|스케줄유형|분석차수|분석차수|분석차수자동증가여부|배치형태|시작일자|스케줄사용여부|스케줄일괄처리여부|스케줄설명|시작시간|시작분|매월|매월값|매주|매주값|매일|매일값|등록유형코드|스케줄ID|"
        ...
        var cols = [                        
                    ...
                    {Type:"Text", 		Width:180,  SaveName:"shdLnm",  Align:"Left", Edit:0, Hidden:0, KeyField:0},
                    ...
                    {Type:"Text",   	Width:50,   SaveName:"anaDgrNm",   	Align:"Center", Edit:0, Hidden:1},
...
function loadDetail(param) {
	$('div#dtlInfo').load('<c:url value="/dq/schedule/ajaxgrid/dqlite_sch_dtl.do"/>', param, function(){
		$('#btnDtlSave').show();
		$('#btnDtlReset').show();
		$('#shdStrHr').show();
		$('#shdStrMnt').show();
		...
		$("#shdStrDtm").val(new Date().toISOString().substring(0, 10));
		//$('#tabs').show();
	});
}
...
function loadDtl(param) {
	$('div#detailInfo').load('<c:url value="/dq/schedule/ajaxgrid/dqlite_sch_batch_dtl.do"/>', param, function(){
...
// 			var param = $("#frmSearch").serialize();
			var param = "shdKndCd="+$("#frmSearch #schdKndCd").val()+"&shdLnm="+$("#frmSearch #shdLnm").val();
	    	grid_sheet.DoSearch("<c:url value="/dq/scheduler/getScheduleList.do" />", param);
	    	break;
	    	
		case 'SearchAllDtl' :	//조회
			var param1 = "shdId="+$("#shdId").val()+"&shdKndCd="+$("#shdKndCd").val();
			grid_sub.DoSearch("<c:url value="/dq/scheduler/getScheduleJobList.do" />", param1);
			break;
...
// 	    	if(ibsSaveJson.data.length == 0) return;  
		  	...
		  	var url = "<c:url value="/dq/scheduler/ajaxgrid/insertSchedule.do"/>";
	    ...
	    case 'Delete' : //삭제
	    	...
	    	var url = "<c:url value="/dq/scheduler/delSchedule.do"/>";
	    	var param = "";
	 	...
	 	case 'DeleteJob' :
	    	...
	    	var url = "<c:url value="/dq/scheduler/delScheduleJob.do"/>";
	    	var param = "";
	    	IBSpostJson2(url, DelJson, param, ibscallback);
...
<!-- 선택 레코드의 내용을 탭처리... -->
	<div id="tabs">
	  ...
	  <div id="tabs-1">
	  	<div id="dtlInfo">
	  		<%@include file="dqlite_sch_dtl.jsp" %> 
	  	</div>
	  </div>
	  <div id="tabs-3">
	  	<%@include file="dqlite_sch_job_lst.jsp" %>
	  </div>
	</div>
	<!-- 선택 레코드의 내용을 탭처리 END -->
</div>
