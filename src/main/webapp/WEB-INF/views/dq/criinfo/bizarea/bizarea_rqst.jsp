...
<html>
<head>
<title><s:message code="BZWR.TRRT.REG.DEMD" /></title><!-- 업무영역 등록요청 -->
...
// 		popup.focus();
	...
	$("#btnRegRqst").click(function(){ 
		...
		if(regchk > 0) {
			if($("#mstFrm #aprLvl").val() > 0 ){
				showMsgBox("CNF", "<s:message code='CNF.SUBMIT' />", 'submitBizAreaLst');
			}else{
    ...
    $("#btnExcelLoad").click( function(){
    	...
    	//팝업 사용 엑셀업로드
    	//검증결과 상세조회 때문에 미사용
    	//var url = '<c:url value="/dq/criinfo/bizarea/popup/bizarea_xls.do"/>';
	...
	$("#btnReqApprove").click(function(){
		...
		if (tmprow > 0 ) {
			...
			var tmphtml = '<s:message code="BZWR.TRRT" />' + ' : ' + param.bizAreaLnm; //업무영역
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headers = [
					{Text:"<s:message code='DQ.HEADER.BIZAREA.RQST'/>", Align:"Center"}
				];
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|업무영역명|레벨|상위업무영역명|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
		...
		var cols = [						
					...
					{Type:"Text",   Width:130,  SaveName:"bizAreaLnm",   	Align:"Left", Edit:1, KeyField:1},
					{Type:"Text",   Width:60,  SaveName:"bizAreaLvl",	Align:"Left", 	 Edit:0},
					{Type:"Text",   Width:130,  SaveName:"uppBizAreaLnm",   	Align:"Left", Edit:1, KeyField:0}, 
...
function doAction(sAction, param)
{
	...
	switch(sAction)
	{
		...
		case "AddWam": //기존 업무영역 추가
			var url = "<c:url value="/dq/criinfo/bizarea/popup/bizarea_pop.do"/>";
			var param = "sflag=CHG";
// 			var popup = OpenWindow("<c:url value="/dq/criinfo/bizarea/popup/bizarea_pop.do?sflag=CHG"/>","bizareaSearch","800","600","yes");
...
// 			alert(saveJson.Code);
			...
			var url = "<c:url value="/dq/criinfo/bizarea/approveBizArea.do"/>";
			var param = $('form[name=mstFrm]').serialize();
...
// 			var param = $("form#frmInput").serialize(); 
			...
			var popwin = OpenModal(url+"?"+param, "bizAreaPop",  800, 600, "no");
			popwin.focus();
			break;
...
// 			var param = "rqstNo="+$("#rqstNo").val();
			...
			grid_sheet.DoSearch("<c:url value="/dq/criinfo/bizarea/getVrfBizAreaListIBS.do" />", param);
		...
		case "VrfDelete" :
			...
			var url = "<c:url value="/dq/criinfo/bizarea/delVrfBizAreaList.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, ibsSaveJson, param, ibscallback);
...
// 			var xlspopup = OpenWindow(url ,"bizareaxls","800","600","yes");
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/dq/criinfo/bizarea/bizarea_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
function ibsToFormFiled(row){
	...
	$("#divTitle").text("<s:message code='BZWR.TRRT' />"+ " : " + grid_sheet.GetCellValue(row,"bizAreaLnm")); //업무영역
	$("#gridRow").val(row);
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/dq/criinfo/bizarea/ajaxgrid/bizarea_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
			    var msg = "<s:message code='MSG.DTL.INFO2' />"; //상세정보 호출중 오류발생...
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title"><s:message code="BZWR.TRRT.REG.DEMD" /></div><!-- 업무영역 등록요청 -->
	</div>
</div>
<!-- 메뉴 메인 제목 -->
<!-- 메뉴 메인 제목 -->
