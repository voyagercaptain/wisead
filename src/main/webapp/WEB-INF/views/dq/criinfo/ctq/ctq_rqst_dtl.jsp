...
<html>
<head>
<title><s:message code="IMCE.INFO.ITEM.REG.NM"/></title><!--중요정보항목등록 상세정보-->
...
// 	}
	...
	$('#btnSearchPop').click(function(event){
 		    	...
 		    	$('#frmInput')[0].reset();
 		    	//팝업 flag 값 설정
				doAction("ctqPopup", "ADD");
		}).parent().buttonset();
	
	$('#btnSearchUppCtqPop').click(function(event){
 		    	event.preventDefault();	//브라우저 기본 이벤트 제거...
 		    	//팝업 flag 값 설정
				doAction("ctqPopup", "UPP");
		});
...
// 		$("form[name=frmInput]")[0].reset();
	...
	$('td').removeClass('ui-buttonset');
	$('button').removeClass('ui-button ui-widget ui-state-default ui-button-text-only ui-corner-left');
	//console.log($('td').hasClass('ui-buttonset'));
});
...
<%-- 	  <input type="hidden" id="stwdId" name="stwdId" value="${result.stwdId}" > --%>
	...
	<div style="clear:both; height:10px;"><span></span></div>
		<div class="stit"><s:message code="IMCE.INFO.ITEM.CON.INFO"/></div><!--중요정보항목정보-->



		<div style="clear:both; height:5px;"><span></span></div>
	<legend><s:message code="IMCE.INFO.ITEM.CON.INFO"/></legend><!--중요정보항목정보-->



	<div class="tb_basic">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='DATA.QLTY.INDC'/>"><!--데이터품질지표-->
					<caption>중<s:message code="IMCE.INFO.ITEM.REG.INFO.REG"/></caption><!--중요정보항목 정보입력-->
					...
					<tbody>
						<tr>
							<th scope="row" class="th_require"><label for="ctqLnm"><s:message code="IMCE.INFO.ITEM.NM"/></label></th><!--중요정보항목명-->

							<td>
								<input type="text" id="ctqLnm" name="ctqLnm" class="wd300"/>
						...
						<tr>
							<th scope="row"><label for="uppCtqLnm"><s:message code="UPRN.IMCE.INFO.ITEM.NM"/></label></th><!-- 상위중요정보항목명 -->

							<td>
								<input type="text" id="uppCtqLnm" name="uppCtqLnm" class="wd300" />
								<button class="btnDelPop" id="btnDelPop"><s:message code="DEL" /></button><!--삭제-->

        						<button class="btnSearchPop" id="btnSearchUppCtqPop"><s:message code="INQ" /></button><!--검색-->
							</td>
						</tr>
						<tr>
