...
<html>
<head>
<title><s:message code="BZWR.TRRT.QLTY.PRES" /></title><!--업무영역별 품질현황-->
...
function initGrid()
{
    with(grid_sheet){
        ...
        var headers = [
                    {Text:"<s:message code='DQ.HEADER.BIZAREAPROG_LST'/>",Align:"Center"}
                ];
                //No.|진단대상ID|진단대상명(물리)|진단대상명|업무영역ID|업무영역명(LNM)|업무영역명|업무규칙수|분석건수|오류건수|품질점수(%)|DPMO|SIGMA
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   	Width:50,  SaveName:"bizAreaId",   	Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   	Width:50,  SaveName:"bizAreaLnm",   	Align:"Left", Edit:0, Hidden:1},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
	    case "Search":
			...
			grid_sheet.DoSearch("<c:url value="/dq/report/bizrule/getBizareaProgQuality.do" />", param);
...
//             progChart.Down2Image({FileName:"BizareaProg", Type:IBExportType.JPEG, Width:800, Url:"./../../../js/IBChart/jsp/Down2Image.jsp"});
            break;
    }
}
...
function chartDraw(strVal, endVal)	{
	...
	for(var i=strVal; i<= endVal; i++) {
		...
		category.push (grid_sheet.GetCellValue(i, "bizAreaLnm"));
	}
	...
	progChart.SetOptions({
		...
		XAxis : {
			...
			Title : { //X축 제목 설정
				Text : "<s:message code='BZWR.TRRT.NM' />"/*업무영역명*/
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title"><s:message code="BZWR.TRRT.QLTY.PRES" /></div><!--업무영역별 품질현황-->
                  ...
                  <tbody>  
                   	<tr>                        
	                    ...
	                    <select id="sortTyp" name="sortTyp" onChange="setSheetColumn()">
		                    <option value="dbConnTrgLnm"><s:message code="DIAG.TRGT.NM" /></option><!--진단대상명-->
