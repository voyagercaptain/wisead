...
$(window).load(function() {
	...
	grid_sheet2.SetCountPosition(0);
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var cols = [                        
                    ...
                    {Type:"Combo",   Width:80,  SaveName:"mapYn", 	   Align:"Center",  Edit:0},             
                    ...
                    {Type:"Text",   Width:130,  SaveName:"sditmPnm",         Align:"Left",    Edit:1, KeyField:1},
                    {Type:"Text",   Width:130,  SaveName:"infotpLnm",	 Align:"Left", Edit:1, Hidden:0}, //도메인명
                    {Type:"Text",   Width:300,  SaveName:"objDescn",	 Align:"Left", 	 Edit:0, ColMerge:0} ,
                    {Type:"Combo",   Width:300,  SaveName:"stndNm",	 Align:"Left", 	 Edit:0,Hidden:1} 
                ];
...
function doAction(sAction)
{
    ...
    {		    
    	...
    	case "Down2Excel":  //엑셀다운로드
    	    ...
    	    var fileName="DB항목 매핑 결과.xlsx";
    		grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, Mode:2, FileName : fileName});
        	break;

        case "SearchMap":
        	var url = '<c:url value="/dq/model/getPdmStndMapList.do"/>';
        ...
        case "SearchAna":
        	var url = '<c:url value="/dq/model/getPdmStndMapAna.do"/>';
        	var param="";
 	 	...
 	 	case 'Save' : //검증
			...
			ibsSaveJson = grid_sheet.GetSaveJson({"AllSave":0,"StdCol":"ibsCheck"}); 

    		//2. 필수입력 누락인 경우
			if (ibsSaveJson.Code == "IBS010") return;
    	
			if(ibsSaveJson.data.length == 0){
				showMsgBox("INF", "<s:message code="ERR.CHKSAVELIST" />");
				return;
			}
 	 	...
 	 	case 'SaveAna' :
 	 	 	if($("#cntArea").is(":visible")){
				ibsSaveJson = grid_sheet2.GetSaveJson(1);
				var cnt = grid_sheet.GetDataLastRow();
				...
				var url = '<c:url value="/dq/stnd/regPdmSditmMapAna.do"/>';
 	 	...
 	 	case 'Delete' :
 	 		ibsDelJson = grid_sheet.GetSaveJson({"AllSave":0,"StdCol":"ibsCheck", ValidKeyField :0}); 
			
 	 		if(ibsDelJson.data.length == 0){
				showMsgBox("INF", "<s:message code="ERR.CHKDEL" />");
				return;
			}

 	 		var url = '<c:url value="/dq/stnd/delPdmSditmMapList.do"/>';
 	 		var param="";

 	 		IBSpostJson2(url, ibsDelJson, param, ibscallback);
...
<%-- 	<button class="btn_excel_down" id="btnExcelLoad" name="btnExcelLoad"><s:message code="EXCEL.UPLOAD" /></button><!-- 엑셀 내리기 --> --%>
    <button class="btn_search" id="btnSearch" 	name="btnSearch"><s:message code="INQ"/></button> <!-- 조회 -->
    <button class="btn_save" id="btnSave" 	name="btnSave"><s:message code="STRG"/></button> <!-- 조회 -->
    ...
    <div style="clear:both; height:10px;"><span></span></div>
	    <div class="grid_01" id="grid_02">
	    	<script type="text/javascript">createIBSheet("grid_sheet2", "100%", "80px");</script>
	    </div>
    </div>
	<!-- 그리드 입력 입력 -->
	    <div style="clear:both; height:5px;"><span></span></div>
