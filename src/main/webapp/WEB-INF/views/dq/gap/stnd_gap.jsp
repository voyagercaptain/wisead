...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":	//요청서 재조회...
        	...
        	var param = $('#frmSearch').serialize();
        	 ...
        	 var url = "";
             if(bizDcd=="DBSDITM"){
            	 url = "<c:url value="/dq/gap/getDBStndItemGapList.do" />"
             }else if(bizDcd=="DBDMN"){
            	 url = "<c:url value="/dq/gap/getDBDmnGapList.do" />"
             }else if(bizDcd=="SDITM"){
            	 url = "<c:url value="/dq/gap/getStndItemGapList.do" />"
             }else if(bizDcd=="DMN"){
            	 url = "<c:url value="/dq/gap/getDmnGapList.do" />"
             }
    ...
    case "Save":	//요청서 재조회...
        ...
        case "Down2Excel":  //엑셀내려받기
            var fileName = "";
            if(bizDcd=="DBSDITM"){
            	fileName = 'DB표준용어 표준적용율 진단.xlsx';
            }else if(bizDcd=="DBDMN"){
            	fileName = 'DB표준도메인 표준적용율 진단.xlsx';
            }else if(bizDcd=="SDITM"){
            	fileName = '기관표준용어 표준적용율 진단.xlsx';
            }else if(bizDcd=="DMN"){
            	fileName = '기관표준도메인 표준적용율 진단.xlsx';
            }
...
<div id="tabs">
	  ...
	  <div id="tabs-DBSDITM">
			<div id="detailInfoDBSDITM"><%@include file="exl/dbstnditemgap_exl.jsp" %></div>
	  </div>
	  <div id="tabs-DBDMN">
			<div id="detailInfoDBDMN"><%@include file="exl/dbstnddmngap_exl.jsp" %></div>
	  </div>
	  <div id="tabs-SDITM">
			<div id="detailInfoSDITM"><%@include file="exl/stnditemgap_exl.jsp" %></div>
	  </div>
	   <div id="tabs-DMN">
			<div id="detailInfoDMN"><%@include file="exl/stnddmngap_exl.jsp" %></div>
	  </div>	 	  
</div>
