...
$(window).resize( function(){
    	//그리드 가로 스크롤 방지
    	grid_changecol.FitColWidth();
    }
);

function initChgGridCol()
{
	...
	with(grid_changecol){
		...
		var cols = [						
					...
					{Type:"Text",   Width:100,  SaveName:"rqstSno",  Align:"Left", ColMerge:1, Edit:0, Hidden:1},
...
y : y좌표
*/
function grid_changecol_OnDblClick(row, col, value, cellx, celly) {
	if(row < 1) return;
}

function grid_changecol_OnClick(row, col, value, cellx, celly) {
	if(row < 1) return;
}

function grid_changecol_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<!-- <body>     -->
	...
	<!-- 그리드 입력 입력 -->
<div class="grid_01">
     <script type="text/javascript">createIBSheet("grid_changecol", "100%", "200px");</script>
</div>
<!-- 그리드 입력 입력 End -->
