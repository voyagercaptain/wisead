...
$(window).resize( function(){
    	//그리드 가로 스크롤 방지
    	grid_change.FitColWidth();
    }
);

function initChgGrid()
{
...
y : y좌표
*/
function grid_change_OnDblClick(row, col, value, cellx, celly) {
	if(row < 1) return;
}

function grid_change_OnClick(row, col, value, cellx, celly) {
	if(row < 1) return;
}

function grid_change_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<!-- <body>     -->
	...
	<!-- 그리드 입력 입력 -->
<div class="grid_01">
     <script type="text/javascript">createIBSheet("grid_change", "100%", "200px");</script>
</div>
<!-- 그리드 입력 입력 End -->
