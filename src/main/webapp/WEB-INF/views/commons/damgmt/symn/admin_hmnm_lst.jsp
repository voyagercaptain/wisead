...
//                     {Text:"<s:message code='COMMON.HEADER.ADMIN.SYMN.LST'/>", Align:"Center"}
                    {Text:"No.|상태|선택|유사어ID|동음이의어명|유사어물리명|유사어구분코드|표준단어ID|대체어논리명|대체어물리명|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|최초요청사용자명|요청일시|요청사용자ID|승인일시|승인사용자ID"} 
                ];
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   Width:40,  SaveName:"symnPnm",    Align:"Left", Edit:1, Hidden:0 ,KeyField:1}, 
                    {Type:"Text",   Width:40,  SaveName:"symnDcd",    Align:"Center", Edit:1, Hidden:1}, 
                    {Type:"Text",   Width:40,  SaveName:"stwdId",    Align:"Left", Edit:0, Hidden:1}, 
                    {Type:"Text",   Width:40,  SaveName:"sbswdLnm",    Align:"Left", Edit:1, Hidden:1}, 
                    {Type:"Text",   Width:40,  SaveName:"sbswdPnm",    Align:"Left", Edit:1, Hidden:1},
...
// 	    SetColProperty("symnTrmDcd", ${codeMap.symnDcdibs});
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/commons/damgmt/symn/ajaxgrid/admin_hmnm_dtl.do"/>', param, function(){
		//$('#tabs').show();
	});
}
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":
			...
			loadDetail();
			$('#symn_sel_title').html('');
			grid_sheet.DoSearch("<c:url value="/meta/stnd/gethmnmList.do" />", param);
			//grid_sheet.DoSearchScript("testJsonlist");
        	break;
       
        case 'SaveRow': //단건 저장
    		//saction (I-입력, U-수정)
    		var urls = '<c:url value="/meta/stnd/savehmnmRow.do"/>';
        ...
        case 'Save' : //엑셀 업로드용 저장
        	...
        	var url = '<c:url value="/meta/stnd/savehmnms.do"/>';
...
//         	setTreeCheckIBS(grid_sheet);
        	...
        	var url = '<c:url value="/meta/stnd/deletehmnmList.do"/>';
        	var param = "";
...
<%--                                 <s:message code="SIMIWORD.NM" /> --%>동음이의어명
                                </label></th> <!-- 유사어명 -->
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li>
	    <a href="#tabs-1">
<%-- 	    <s:message code="SIMIWORD.PRHB.WORD.REG" /> --%>
	    동음이의어 상세정보
	     </a>
