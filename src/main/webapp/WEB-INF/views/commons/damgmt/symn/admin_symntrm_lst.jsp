...
// 	alert('window.load');
	...
	if(symnId != null && symnId != "" && symnId != "undefined") {
		grid_sheet.DoSearch("<c:url value="/meta/stnd/getSymntrmList.do" />", param);
...
//                     {Text:"<s:message code='COMMON.HEADER.ADMIN.SYMN.LST'/>", Align:"Center"}
                     {Text:"No.|상태|선택|유사어ID|유사어/금칙어 명|유사어물리명|구분코드|표준단어ID|대체어논리명|대체어물리명|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|최초요청사용자명|요청일시|요청사용자ID|승인일시|승인사용자ID"}
                ];
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   Width:40,  SaveName:"symnLnm",    Align:"Left", Edit:1, Hidden:0 ,KeyField:1}, 
                    {Type:"Text",   Width:40,  SaveName:"symnPnm",    Align:"Left", Edit:1, Hidden:1}, 
                    {Type:"Combo",   Width:40,  SaveName:"symnDcd",    Align:"Center", Edit:1, Hidden:0 ,KeyField:1}, 
                    {Type:"Text",   Width:40,  SaveName:"stwdId",    Align:"Left", Edit:0, Hidden:1}, 
                    {Type:"Text",   Width:40,  SaveName:"sbswdLnm",    Align:"Left", Edit:1, Hidden:0 }, 
                    {Type:"Text",   Width:40,  SaveName:"sbswdPnm",    Align:"Left", Edit:1, Hidden:0 },
                    ...
                    {Type:"Date",   Width:40,  SaveName:"frsRqstDtm",    Align:"Center", Edit:0, Hidden:0, Format:"yyyy-MM-dd HH:mm:ss"}, 
                    {Type:"Text",   Width:40,  SaveName:"frsRqstUserId",    Align:"Left", Edit:0, Hidden:1}, 
                    {Type:"Text",   Width:40,  SaveName:"frsRqstUserNm",    Align:"Left", Edit:0, Hidden:0},
	    ...
	    SetColProperty("symnDcd", ${codeMap.symnTrmDcdibs});
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/commons/damgmt/symn/ajaxgrid/admin_symntrm_dtl.do"/>', param, function(){
		//$('#tabs').show();
	});
}
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case 'SaveRow': //단건 저장
    		//saction (I-입력, U-수정)
    		var urls = '<c:url value="/meta/stnd/saveSymnTrmRow.do"/>';
        ...
        case 'Save' : //엑셀 업로드용 저장
        	...
        	var url = '<c:url value="/meta/stnd/saveSymnsTrm.do"/>';
...
//         	setTreeCheckIBS(grid_sheet);
        	...
        	var url = '<c:url value="/meta/stnd/deleteSymnTrmList.do"/>';
        	var param = "";
...
<%-- 	    <s:message code="SIMIWORD.PRHB.WORD.MNG" /> --%>
	    유사어/금칙어 관리
	    </div> <!-- 유사어/금지어 관리 -->
	</div>
</div>
        ...
        <form id="frmSearch" name="frmSearch" method="post">
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='STRD.WORD.INQ' />"> <!-- 표준단어조회 -->
                   ...
                   <tbody>                            
                            <tr>
                                <th scope="row"><label for="symnLnm">
<%--                                 <s:message code="SIMIWORD.NM" /> --%>
                                유사어/금칙어 명
                                </label></th> <!-- 유사어명 -->
                                <td><input type="text" id="symnLnm" class="wd300" name="symnLnm" /></td>
                                <th scope="row"><label for="symnDcd">
<%--                                 <s:message code="SIMIWORD.DV.CD" /> --%>
                                구분코드
                                </label></th> <!-- 유사어구분코드 -->
                                <td><select id="symnDcd" class="wd300" name="symnDcd">
                                       <option selected="" value=""><s:message code="WHL" /></option> <!-- 전체 -->
					                	<c:forEach var="code" items="${codeMap.symnTrmDcd}" varStatus="status">
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1"><s:message code="SIMIWORD.PRHB.WORD.REG" /></a></li> <!-- 유사어/금지어 등록 -->
