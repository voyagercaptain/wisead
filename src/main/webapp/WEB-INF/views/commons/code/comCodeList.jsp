...
//	    	colNames:['<input type="checkbox" onclick="checkBox(event)" />', '프로그램파일명', '프로그램경로', '프로그램명', 'URL', '프로그램설명'],
	   	colNames:[<s:message code='COMMON.HEADER.COMCODELIST'/>], /* '코드분류', '공통코드ID', '공통코드명', '공통코드설명', '사용여부', '분류코드' */
	   	colModel:[
//	    		{name:'chkbox',index:'chkbox', width:55, sortable:false, edittype:'checkbox', editoptions: { value:"Yes:No" }, editable:true},
	   		{name:'clCodeNm',index:'clCodeNm', width:55, sorttype:'text', editable:true},
	   		{name:'codeId',index:'codeId', width:55, sorttype:'text', editable:true},
	   		{name:'codeIdNm',index:'codeIdNm', width:55, sorttype:'text', editable:true},
	   		{name:'codeIdDc',index:'codeIdDc', width:90, sorttype:'text', editable:true},
	   		{name:'useAt',index:'useAt', width:100, sorttype:'text', editable:true},
	   		{name:'clCode',index:'clCode', width:150, sortable:false, hidden:true, editable:true}		
	   	],
	    rowNum:100,
...
//	 		records:'records', 		// 보여지는 데이터 갯수(레코드) totalRecord
			id:'codeId' 		// 키값, 꼭 화면 전체에서 겹치는 값이 없는 column을 설정할 것, 안그러면 에러 발생
		}, 
		loadonce: true,
		scroll: 1,
		multiselect: true,
...
// 		$("#gridlist01").jqGrid('clearGridData'); // 이전 데이터 삭제
		$("#gridlist01").jqGrid('setGridParam', {
			url:'<c:url value="/commons/code/ajaxgrid/selectComCodeList.do" />'+'?'+param,
// 			postData: param,
			datatype:'json'
		}).trigger("reloadGrid");
		break;
	...
	case 'SaveRow': //단건 저장
		//saction (I-입력, U-수정)
		var urls = '<c:url value="/commons/code/ajaxgrid/saveComCode.do"/>';
	...
	case 'Delete' : //삭제
		var sel = $("#gridlist01").jqGrid('getGridParam','selarrrow');
		//url 호출
		var urls = '<c:url value="/commons/code/ajaxgrid/deleteComCode.do"/>';
		var param = "codeId="+sel;
	...
	case 'ExcelDown' : //엑셀다운로드
		...
		$("#frmExcel input[name=excelname]").val("<s:message code='COM.CD.LST.XLS' />");	//엑셀 파일 명... // 공통코드리스트.xls
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/commons/code/ajaxgrid/selectComCodeDetail.do"/>', param, function(){});
...
// 	alert(data.result);
	if(data.result > 0) {
		...
		//그리드에 추가 또는 업데이트
		if("I" == data.saction) {
			$("#gridlist01").addRowData(data.resultVO.codeId , data.resultVO, 'first');
		} else if ("U" == data.saction) {
			$("#gridlist01").setRowData(data.resultVO.codeId, data.resultVO);
