...
//	    	colNames:['<input type="checkbox" onclick="checkBox(event)" />', '프로그램파일명', '프로그램경로', '프로그램명', 'URL', '프로그램설명'],
	   	colNames:[<s:message code='COMMON.HEADER.LOGINLOGLIST'/>], /* '로그ID', '접속자', '접속ID',  '접속IP', '접속유형',  '접속일자' */
	   	colModel:[
//	    		{name:'chkbox',index:'chkbox', width:55, sortable:false, edittype:'checkbox', editoptions: { value:"Yes:No" }, editable:true},
	   		{name:'logId',   index:'logId'  , width:55, sortable:false, editable:true, hidden:true},
	   		...
	   		{name:'loginIp', index:'loginIp', width:90, sorttype:'text', editable:true},
	   		{name:'loginMthd',index:'loginMthd', width:100, sorttype:'text', editable:true},
	   		{name:'creatDt', index:'creatDt', width:150, sortable:true, sorttype:'text'}		
	   	],
	    rowNum:100,
...
//	 		records:'records', 		// 보여지는 데이터 갯수(레코드) totalRecord
			id:'logId' 		// 키값, 꼭 화면 전체에서 겹치는 값이 없는 column을 설정할 것, 안그러면 에러 발생
		}, 
		loadonce: true,
		scroll: 1,
		sortable: true,
// 		multiselect: true,
	...
	/*     loadComplete : function () {
			   $("#logingrid01").jqGrid('setGridParam',{datatype:'local'});
		}, */
//	  	onPaging : function(which_button) {
//	 		   $("#logingrid01").jqGrid('setGridParam',{datatype:'json'});
//	 		   $("#btnSearch").click();
//	 	}
	   	onSelectRow: function(id){
	 		...
	 		/*  if(id && id!==lastsel){
	 			var ret = $("#logingrid01").jqGrid('getRowData',id);
...
//	  		return false;
	...
	/* 		if(id && id!==lastsel){
				$('#logingrid01').jqGrid('saveRow', lastsel, false, 'clientArray');
				$('#logingrid01').jqGrid('editRow', id, true, null, null, 'clientArray');
				lastsel=id;
			} */
		}, 
...
//	 		alert("onCellSelect - " + id + ":" + icol + ":" + cont);
	...
	// $("#logingrid01").jqGrid('navGrid','#loginpager01',{edit:false,add:false,del:false});
}
...
// 			postData: param,
	...
	case 'ExcelDown' : //엑셀다운로드
		//그리드 내용을 가져온다.
		var tbody = $("#logingrid01").clone(); //.html();
		//그리드 헤더내용을 가져온다.
		...
		$("#frmExcel input[name=excelname]").val("<s:message code='CONN.LOG.XLS' />");	//엑셀 파일 명... /* 접속로그.xls */
		$("#frmExcel").attr('action', url).submit();
		
		//tableToExcel('logingrid01', 'test');
...
<%-- 	<div class="location"><img src='<c:url value="/img/location_home.gif"/>' alt="home"> &gt; 관리자 &gt; 프로그램관리</div> --%>
    <div class="stit"><s:message code="USER.CONN.HSTR.INQ" /></div> <!-- 사용자 접속이력 조회 -->
...
--%>
    ...
    <table id="logingrid01"></table>
	<div id="loginpager01"></div>
    </div>
   <!-- 그리드 입력 입력 -->
