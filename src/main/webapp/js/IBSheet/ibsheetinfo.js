 ...
 */
function IBS_Sheet2SheetCheck(fromSheet, toSheet, chkCol, shdId)  {
...
//  alert(chkCol);
  ...
  for (var ir = 1; ir<=fromRow; ir++) {
    ...
    if(toRow > 1){
	    ...
	    var toVal = fromSheet.GetCellValue(fromRow, 5);
	    if(cdVal != toVal){
    ...
    for (var ic = 0; ic<=fromSheet.LastCol(); ic++) {
      //시퀀스 설정
      if (ic == 0) toSheet.SetCellValue(toRow, 1, toRow);
      //체크 컬럼, 상태 컬럼은 빼고 옮김
      if ( ic == 1 || ic ==2 ) continue;
    
      if ( ic == 3 ){
    	  toSheet.SetCellValue(toRow, ic, shdId);
      }else{
