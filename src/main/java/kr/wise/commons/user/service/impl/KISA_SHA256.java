...
public class KISA_SHA256 {
	...
	*/
	public static void SHA256_Process( SHA256_INFO Info, byte[] pszMessage, int uDataLen ) {
		...
		int dataLen = uDataLen;
		
		if((Info.uLowLength += (dataLen << 3)) < 0) {
			Info.uHighLength++;
		}

		Info.uHighLength += Common.URShift(dataLen,29);
		...
		while (uDataLen >= SHA256_DIGEST_BLOCKLEN) {
			...
			dataLen -= SHA256_DIGEST_BLOCKLEN;
		}

		Common.arraycopy_offset(Info.szBuffer, 0, pszMessage, pszMessage_offset, dataLen);
	}
	...
	*/
	public static String SHA256_Encrpyt(String str) {
		byte pbCipher[]   = new byte[32];
		int uPlainTextLen = str.length();
		byte[] pszMessage = str.getBytes();
		...
		if(pszMessage.length > 0){
			...
			SHA256_Close( info, pbCipher );
			
			for (int i=0; i<32; i++){
				result.append(Integer.toHexString(0xff&pbCipher[i]));
			}
		}
