 ...
 * </pre>
 */
public abstract class AbstractIdGnrService implements EgovIdGnrService,
     ...
     */
    private static final BigDecimal BIG_DECIMAL_MAX_LONG =
    ...
    /**
     * 로거 얻기 메소드
     * @return Log IdGenerationService logger
     */
    protected Log getLogger() {
        return EgovIdGnrService.LOGGER;
    }
    ...
    protected final long getNextLongIdChecked(long maxId) throws Exception {
        ...
        if (useBigDecimals) {
            ...
            if (bd.compareTo(BIG_DECIMAL_MAX_LONG) > 0) {
                ...
                throw new Exception("error.idgnr.greater.maxid");
            }
            nextId = bd.longValue();
        } else {
            // Use long data type
     ...
     */
    public final String getNextStringId() throws Exception {
    ...
    public String getNextStringId(String strategyId) throws Exception {
        this.strategy =
            (EgovIdGnrStrategy) this.beanFactory.getBean(strategyId);
        return getNextStringId();
    }
