package kr.wise.advisor.prepare.udefoutlier.ocv.service.impl;
...
@Service("UodcOcvService")
public class UodcOcvServiceImpl implements UodcOcvService {
	...
	@Inject
	private WadUodcOcvMapper wadUodcOcvMapper;
	...
	public WadUodcOcv getUodcOcvDetail(WadUodcOcv search) {
		...
		return wadUodcOcvMapper.selectDetailInfo(search);
	}

	@Override
	public List<WadUodcOcv> getUodcOcvColList(WadUodcOcv search) {
		// TODO Auto-generated method stub
		return wadUodcOcvMapper.selectOcvColList(search);
	}

	@Override
	public int regUodcOcv(List<WadUodcOcv> list, WadUodcOcv mstData) throws Exception {
		// TODO Auto-generated method stub
		int result = 0;
		...
		creVo.setCreCompNm("One Class SVM");
		creVo.setCompId("ODOCV");
		...
		//기존 데이터 UPDATE
		result = wadUodcOcvMapper.updateOcv(mstData);
		
		if(result == 0){
			//WAD_UODC_DASE_JN data import mst Insert
			result = wadUodcOcvMapper.insertOcv(mstData); 					
		}
		...
		// 분석데이터셋ID  UPDATE 		
		result = wadUodcOcvMapper.updateOcvAnaDaseId(mstData);
		result = wadUodcOcvMapper.updateOcvAnaDaseColId(mstData);
		
		return result;
	}

	@Override
	public List<WadUodcOcv> getUodcOcvColLstForScrt(WadUodcOcv search) {

		return wadUodcOcvMapper.selectUodcOcvColLstForScrt(search); 
	}
